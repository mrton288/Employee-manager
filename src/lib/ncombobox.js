class Combobox {
    ...
    async buildComboboxHTML() {
        ...
        for (const combobox of comboboxs) {
            ...
            const tabCombobox = combobox.getAttribute("tabindex");

            // Build template html của combobox
            let comboboxHTMLTemplate = `<input></input>
                                        <datacbx></datacbx>
                                        <buttoncbx>
            ...
            comboboxHTMLNew.setAttribute("tabindex", tabCombobox || "");
            comboboxHTMLNew.innerHTML = comboboxHTMLTemplate;
            // Tạo element input

            let inputCombobox = comboboxHTMLNew.querySelector("input");
            inputCombobox.setAttribute("id", combobox.querySelector("input").getAttribute("id"));
    ...
    builDataCombobox(dataCombobox, inputCombobox) {
        ...
        try {
            ...
            for (const item of this.Data) {
                ...
                liElement.onclick = () => {
                    inputCombobox.value = item.DepartmentName;
                    dataCombobox.classList.add("hide");
                };
            }

            inputCombobox.addEventListener("input", function (e) {
                let textSearch = e.target.value.trim().toLowerCase();
                let listLi = document.querySelectorAll("li.option1");
                console.log(listLi);
                listLi.forEach((element) => {
                    if (element.innerText.toLowerCase().includes(textSearch)) {
    ...
    searchInputCombobx(inputCombobox) {
        try {
        } catch (error) {
            console.log(error);
        }
